...
class State {
    ...
    std::string nev;
public:
    //State() : nev(nullptr), elfogado('H') {}
    char get_elfogado() const { return elfogado; }
    const char* get_nev() const { return nev.c_str(); }
    void set_elfogado(char c) { elfogado = c; }
    void set_nev(std::string s) { nev = s; }
};
